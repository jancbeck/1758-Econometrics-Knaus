...
```
According to the estimates of Model 2, food expenditure is expected to increase by 0.49 franc for every additional franc of income.
...
```
The summary of the linear regression of Model 2, where logged food expenditure is the dependent variable and logged household income is the explanatory variable shows us that the estimate for the intercept $\hat{\beta}_{0}$ is __`r m2_B0`__ and for the slope $\hat{\beta}_{1}$ it is __`r m2_B1`__.
...
```
According to Model 2, we can say that for every percentage increase of income, food expenditure is expected to increase by _0.84%_.
...
pred_foodexp2 <-m2_B0 + m2_B1 * 10000

```

For model1 $\hat{\beta}_{0}$ is 147.47539 and $\hat{\beta}_{1}$ is 0.48518. The formula for model1 is: $$Y = {\beta}_{0} + {\beta}_{1}X + u, \,\, \mathbb{E}[u|Y] = 0  $$ If we set  X (income) to 10000 we get an expected food expenditure of `r pred_foodexp1` ~ 5000 franc. \
...
<!-- power point doesnt show vice versa. We have to proof both directions -->
